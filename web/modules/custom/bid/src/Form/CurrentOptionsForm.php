...
class CurrentOptionsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    if ( isset($_GET['op']) && $_GET['op'] == 'remove' ) {
      ...
      $form['cancel'] = array(
        '#type' => 'submit',
        '#attributes' => array('class' => array('btn-secondary', 'btn-sm', 'btn-retirar_oferta')),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('op') == 'Editar oferta') {
      ...
      $bid = views_get_view_result('mis_ofertas', 'default', $user->id(), $carga->id());
      $n = Node::load($bid[0]->nid);
      $n->delete();
